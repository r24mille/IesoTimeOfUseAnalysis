function [ weekday_par_ts ] = par_by_weekday( demand_ts )
...
for i = 0:(num_days - 1)
    ...
    if any(ont_holidays == floor(starttime))
        % disp(['Skipped Holiday, ', datestr(floor(starttime)), '.'])
    elseif weekday(floor(starttime)) == 1 || weekday(floor(starttime)) == 7
        % disp(['Skipped Weekend, ', datestr(floor(starttime)), '.'])
    else
        % starttime is non-holiday weekday, continue
...
weekday_par_ts = timeseries(daily_pars(:,1), datestr(daily_pars(:,2)), ...
    'Name', 'Daily PAR (Weekdays)');
end
